/*
Данная задача является бонусной высокого уровня, она гарантирует вам до +1.5 балла к оценке семинариста при чистом решении к 10-бальной оценке в конце
семестра в случае, если ваша оценка на тот момент будет >= уд(3).

Решать данную задачу на интерпретируемых языках не имеет смысла, так как они дадут просадку производительности "из коробки" и на практике весь подобный код на таких языках не пишется.
Желательно - С++, но готов рассматривать Go, Rust, C, Java.
*/
...
/*
    ...
    2. Точное решение. Например, можно достаточно эффективно свести данную задачу к задаче коммивоежера. Или к другой известной NP полной задаче.
    ...
    4. Использование систем непрерывной интеграции (CI), например https://travis-ci.org/ или github actions. Вообще, travis/другой CI должен стать вашим другом 
        при написании чего-то, содержащего более 200 строк кода.
*/
